    ...
    function drawingInit(){restore();if(!drawingState.assemblyView)drawingState.assemblyView='combined';if(!drawingState.outputAssemblyView)drawingState.outputAssemblyView='combined';drawingState.cabinets.forEach(c=>{if(!["1.0T","1.2T","1.4T","1.6T"].includes(c.thickness))c.thickness="1.6T"});if(!drawingState.stack3dView)drawingState.stack3dView={yaw:-35,pitch:-18,zoom:1,panX:0,panY:0};if(!drawingState.productionNotes)drawingState.productionNotes={memo:""};buildTools();syncInputs();syncProductionNotesToInputs();document.querySelectorAll('[data-assembly-mode]').forEach(x=>x.classList.toggle('active',x.dataset.assemblyMode===drawingState.assemblyView));document.querySelectorAll('[data-output-assembly]').forEach(x=>x.classList.toggle('active',x.dataset.outputAssembly===drawingState.outputAssemblyView));renderAll();drawingGenerateBtn.onclick=applyDimensions;drawingApplyBtn.onclick=applyObj;drawingDeleteBtn.onclick=deleteObj;drawingSaveBtn.onclick=()=>openShareModal("drawing-save");$("drawingShareBtn").onclick=()=>openShareModal("drawing-share");drawingResetBtn.onclick=reset;drawingExportBtn.onclick=exportPng;$("stackAddBtn").onclick=addCabinet;$("stackCloneBtn").onclick=cloneCabinet;$("stackDeleteBtn").onclick=deleteCabinet;$("cabinetApplyBtn").onclick=applyCabinetProps;document.querySelectorAll("[data-mode]").forEach(b=>b.onclick=()=>{drawingState.mode3d=b.dataset.mode;document.querySelectorAll("[data-mode]").forEach(x=>x.classList.toggle("active",x===b));render3d()});document.querySelectorAll('[data-assembly-mode]').forEach(b=>b.onclick=()=>{drawingState.assemblyView=b.dataset.assemblyMode;document.querySelectorAll('[data-assembly-mode]').forEach(x=>x.classList.toggle('active',x===b));drawingState.selectedObjectId=null;renderAll();setStatus(b.textContent+' 보기로 전환했습니다.')});document.querySelectorAll('[data-output-assembly]').forEach(b=>b.onclick=()=>{drawingState.outputAssemblyView=b.dataset.outputAssembly;document.querySelectorAll('[data-output-assembly]').forEach(x=>x.classList.toggle('active',x===b));document.dispatchEvent(new CustomEvent('kenc:preview-invalidated'));renderAll()});document.querySelectorAll("[data-stack-preview]").forEach(b=>b.onclick=()=>{drawingState.stackPreview=b.dataset.stackPreview;document.querySelectorAll("[data-stack-preview]").forEach(x=>x.classList.toggle("active",x===b));renderStackPreview()});[drawingObjectX,drawingObjectY,drawingObjectW,drawingObjectH,drawingObjectRotation].forEach(i=>i.onchange=applyObj);drawingObjectLabel.oninput=()=>{const o=selectedObj();if(o&&o.type==="nameplate"){o.label=drawingObjectLabel.value;renderAll()}};["productionMemo"].forEach(id=>$(id).addEventListener("input",syncProductionNotesFromInputs))}
