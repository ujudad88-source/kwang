(function(){
  ...
  function basisFor(c,o,s,sc,y0){
    const w=c.width,h=c.height,d=c.depth,x=o.x||0,y=o.y||0;
    let O,U,V;
    if(s==='front'){O=p(-w/2+x,y0-h/2+y,d/2+3,sc);U={x:sc,y:0};V={x:0,y:sc};}
  ...
  function objectRect(svg,c,o,s,sc,y0){
    const b=basisFor(c,o,s,sc,y0); if(!b)return;
  ...
  function render(state,cabinet){
    ...
    const c=cabinet,w=+c.width,h=+c.height,d=+c.depth,sc=Math.min(430/w,470/h,150/d),y0=0;
    ...
    svg.appendChild(el('polygon',{points:[A,B,C,D].map(q=>`${q.x},${q.y}`).join(' '),fill:'#f8fafc',stroke:'#111827','stroke-width':3}));
    ...
    const order=['back','inside','left','right','top','bottom','front'];order.forEach(s=>(c.objects||[]).filter(o=>(o.surface||'front')===s).forEach(o=>objectRect(svg,c,o,s,sc,y0)));
