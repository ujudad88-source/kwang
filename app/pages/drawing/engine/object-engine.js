(function(){
  ...
  function normalizeState(s){if(!s||typeof s!=='object')return s;s.cabinets=Array.isArray(s.cabinets)?s.cabinets:[];s.cabinets.forEach((c,i)=>{normalizeCabinet(c);window.KENC_CABINET_ENGINE?.normalize(c,i);(c.objects||[]).forEach(o=>{window.KENC_UUID?.ensure(o,'uuid');window.KENC_MATERIAL_ENGINE?.normalizeObject(o);});});s.engineVersion='2.0.0';return s;}
  ...
  window.KENC_OBJECT_ENGINE={version:'2.0.0',definition,resolveVariant,normalizeObject,normalizeCabinet,normalizeState,plane,clampObject,addObject,updateObject,removeObject,serialize,deserialize,on,emit};
})();
